...
class DetailActivity : AppCompatActivity() {
    ...
    lateinit var tv_pesan: TextView
    var ramenList = ArrayList<Ramen>()
    ...
    lateinit var mDatabaseRestoReference: DatabaseReference
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mDatabaseRestoReference = mFirebaseInstance.getReference("Restaurant1").child(data?.nama.toString()).child("Ramen")
        //mDatabaseRestoReference = mFirebaseInstance.getReference("Restaurant1").child(data?.nama.toString()).child("Ramen")
        rv_menu = findViewById(R.id.rv_menu)
        tv_pesan = findViewById(R.id.tvpesan)
...
//        et_Drink2.isEnabled = false
        ...
        tv_pesan.setText(data?.nama)
        println(data?.alamat)
        println(data?.nama)
//        rv_menu.adapter = MenuAdapter(data?.ramen){
//            println(it.nama)
//        }
...
//        }
    ...
    private fun getData() {
        mDatabaseRestoReference.addValueEventListener(object : ValueEventListener{
            override fun onDataChange(dataSnapshot: DataSnapshot) {

                ramenList.clear()
                for (getdataSnapshot in dataSnapshot.children) {
                    var ramen = getdataSnapshot.getValue(com.example.tubes.model.Ramen::class.java)
                    println(ramen?.nama)
                    ramenList.add(ramen!!)
                }

                rv_menu.adapter = MenuAdapter(ramenList) {
                }
            }
